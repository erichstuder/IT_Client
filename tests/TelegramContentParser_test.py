...
def test_parseString_noTelegram():
	with pytest.raises(TelegramContentParserException, match='telegram is None'):
		_TelegramContentParser.parseString(None, [])

def test_parseString_noStream():
	with pytest.raises(TelegramContentParserException, match='string has no terminator'):
		_TelegramContentParser.parseString({}, [])

def test_parseString():
	string = 'dummy'
	telegram = {}
	newStream = _TelegramContentParser.parseString(telegram, [ord(c) for c in string] + [0] )
	assert telegram['value'] == string
	assert newStream == []
